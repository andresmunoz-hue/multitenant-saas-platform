"""Gold layer: daily metrics by delivery type."""
...
from saas_pipeline.paths import gold_metrics, silver_fact
...
def run_gold(
    ...
    # Recompute partitions in range: overwrite matching fecha_proceso values
    dates = [r.fecha_proceso for r in metrics.select("fecha_proceso").distinct().collect()]
    if not dates:
        # Still ensure empty table schema exists
        (
